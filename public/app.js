...
function renderFeatures(config) {
  ...
  const managed = [
    ...
    feature('CAL', 'Speed, heading, and wind calibration advice', config.calibrationAdvisor?.enabled, config.calibrationAdvisor?.enabled ? `${config.calibrationAdvisor.minimumSamples} speed / ${config.calibrationAdvisor.headingMinimumSamples} heading samples · wind procedure below` : 'Read-only ST60 estimates and wind procedure')
  ]
